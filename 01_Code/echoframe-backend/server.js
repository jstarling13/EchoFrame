...
app.use(cors({
  ...
  credentials: false,  // explicit: never forward cookies or auth headers cross-origin
}));

app.use(express.json());

// ── Static legal pages ────────────────────────────────────────────────────────
// Serves terms.html and privacy.html at /terms.html and /privacy.html
// These URLs can be referenced in Stripe Dashboard ToS settings.
...
app.post('/create-checkout-session', checkoutLimiter, async (req, res) => {
  try {
    ...
    // ── ROSCA / CAN-SPAM compliance: require ToS acceptance at checkout ──────
    // TODO (manual): In Stripe Dashboard → Settings → Public business info,
    //   set your Terms of Service URL so the checkbox links to it correctly.
    //   Also enable "Require customers to accept your terms of service" on each
    //   buy.stripe.com Payment Link (Checkout → Advanced options) — that part
    //   cannot be set via API and must be toggled in the dashboard per link.
    const session = await stripe.checkout.sessions.create({
      line_items: [{ price: product.priceId, quantity: 1 }],
      mode: product.mode,
      success_url: `${process.env.CLIENT_URL}/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url:  `${process.env.CLIENT_URL}/cancel`,
      // Require explicit ToS acceptance before payment (ROSCA compliance)
      consent_collection: {
        terms_of_service: 'required',
      },
      custom_text: {
        terms_of_service_acceptance: {
          message: 'I agree to the [EchoFrame Terms of Service](https://echoframe-production.up.railway.app/terms.html) and acknowledge that reports are informational only and do not constitute financial, tax, or legal advice.',
        },
      },
    });
