...
2. Open **API Keys → Create API Key**. Name it for White Oak Preview, select sending-only permission, and restrict it to the intended domain when available. Store the displayed key securely; this supplies `EMAIL_PROVIDER_API_KEY`. See [Resend's API-key instructions](https://resend.com/docs/create-an-api-key).
