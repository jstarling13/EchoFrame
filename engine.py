...
def _generate_narrative(meta: dict, metrics: dict, leaks: list) -> dict:
    ...
    user_message = (
        ...
        + f"Owner context: {context if context else 'None provided.'}"
...
def _set_para_shading(paragraph, hex_color: str) -> None:
...
def _save_report(email: str, docx_bytes: bytes, meta: dict) -> str:
    REPORTS_DIR.mkdir(exist_ok=True)
    biz_slug  = re.sub(r"[^a-z0-9]+", "_", meta.get("Business Name", "report").lower()).strip("_")
    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    path      = REPORTS_DIR / f"EchoFrame_{biz_slug}_{timestamp}.docx"
...
def _send_report_email(email: str, name: str, docx_bytes: bytes, meta: dict):
    resend.api_key = os.environ.get("RESEND_API_KEY")
    if not resend.api_key:
        print("[EchoFrame] ERROR - RESEND_API_KEY not set. Email skipped.")
        return
...
<div style="max-width:560px;margin:40px auto;background:#FFFFFF;border-radius:12px;overflow:hidden;border:1px solid #E5E7EB;">
  ...
  <div style="background:#012A4A;padding:28px 36px;">
    <div style="color:#FFFFFF;font-size:22px;font-weight:700;letter-spacing:-0.02em;">EchoFrame</div>
    <div style="color:#93C5FD;font-size:13px;margin-top:4px;">Monthly Clarity Report &mdash; {month}</div>
  </div>

  <div style="padding:36px 36px 28px;">
    <p style="color:#111827;font-size:16px;font-weight:600;margin:0 0 16px 0;">Hi {name},</p>
    <p style="color:#4B5563;font-size:15px;line-height:1.65;margin:0 0 20px 0;">
      Attached is your Monthly Clarity Report. Please review the
      <strong style="color:#012A4A;">Top 5 Expense Leaks</strong> and the recommended
      <strong style="color:#012A4A;">Next Steps</strong> &mdash; those two sections
      contain the highest-leverage actions for your business this month.
    </p>
    <p style="color:#4B5563;font-size:15px;line-height:1.65;margin:0;">
      Let me know if you have any questions. I&rsquo;m here to help you act on the numbers,
      not just read them.
    </p>
  </div>

  <div style="padding:0 36px 36px;">
    <div style="background:#F0F4FF;border-left:3px solid #012A4A;border-radius:0 8px 8px 0;padding:16px 20px;font-size:14px;color:#374151;line-height:1.7;">
      <strong style="color:#012A4A;">Your report covers:</strong><br>
      Executive Summary &nbsp;&bull;&nbsp; Revenue Analysis &nbsp;&bull;&nbsp; Top 5 Expense Leaks<br>
      Cash Flow Snapshot &nbsp;&bull;&nbsp; 90-Day Projection &nbsp;&bull;&nbsp; The One Thing &nbsp;&bull;&nbsp; Next Steps
    </div>
  </div>

  <div style="padding:18px 36px;border-top:1px solid #E5E7EB;background:#F9FAFB;">
    <p style="color:#9CA3AF;font-size:12px;margin:0;">
      EchoFrame &nbsp;&middot;&nbsp; Plain English Financial Reports &nbsp;&middot;&nbsp; echoframe.co
    </p>
  </div>
    ...
    try:
        params: resend.Emails.SendParams = {
            "from":    "EchoFrame <onboarding@resend.dev>",
            "to":      [email],
            "subject": "Your EchoFrame Financial Clarity Report",
            "html":    html,
            "attachments": [{
                "filename": "EchoFrame_Clarity_Report.docx",
                "content":  list(docx_bytes),
            }],
        }
        response = resend.Emails.send(params)
        email_id = response.id if hasattr(response, "id") else response.get("id", "?")
        print(f"[EchoFrame] Email delivered -> {email}  (id: {email_id})")
    except Exception as e:
        print(f"[EchoFrame] ERROR - email delivery failed for {email}: {e}")


# ── Helpers ───────────────────────────────────────────────────────────────────

def _safe_email_legacy(email: str) -> str:
    # Kept for reference only — use module-level _safe_email instead
